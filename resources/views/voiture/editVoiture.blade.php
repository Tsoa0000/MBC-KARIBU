                    ...
                    @foreach ($voiture as $v)
                        <tr data-id="{{ $v['id'] }}">
                            <td>
                                ...
                                <input class="edit-mode form-control form-control-sm d-none" type="text" name="matricule"
                                    value="{{ $v['matricule'] }}" />
                            </td>
                            <td>
                                <span class="view-mode">{{ $v['modele'] }}</span>
                                <input class="edit-mode form-control form-control-sm d-none" type="text" name="modele"
                                    value="{{ $v['modele'] }}" />
                            </td>
                            <td>
                                <span class="view-mode">{{ $v['typeVehi'] }}</span>
                                <input class="edit-mode form-control form-control-sm d-none" type="text" name="typeVehi"
                                    value="{{ $v['typeVehi'] }}" />
                            </td>
                            <td>
                                <span
                                    class="badge bg-{{ $v['etat'] >= 7 ? 'success' : ($v['etat'] >= 4 ? 'warning' : 'danger') }} view-mode">
                                    {{ $v['etat'] }}/10
                                </span>
                                <input class="edit-mode form-control form-control-sm d-none" type="number" min="0"
                                    max="10" name="etat" value="{{ $v['etat'] }}" />
                            </td>
                            <td>
                                <span class="view-mode">{{ $v['conso'] }}</span>
                                <input class="edit-mode form-control form-control-sm d-none" type="number" name="conso"
                                    value="{{ $v['conso'] }}" />
                            </td>
                            <td>
                                <span class="view-mode">{{ $v['nbrPlace'] }}</span>
                                <input class="edit-mode form-control form-control-sm d-none" type="number" name="nbrPlace"
                                    value="{{ $v['nbrPlace'] }}" />
                            </td>
                ...
                .then(json => {
                    if (json.success) {
                        ...
                        if (badge) {
                            ...
                            badge.className = 'badge bg-' + (etat >= 7 ? 'success' : (etat >= 4 ? 'warning' :
                            'danger'));

...
@section('body')
    <main id="main" class="main">
        <div class="container">
            <div class="card">
                <h2 class="page-title">Consommation carburant</h2>
                @if (session('success'))
                    <div class="alert alert-success">{{ session('success') }}</div>
                @endif

                <form action="{{ route('consommation.store') }}" method="POST" class="form">
                    @csrf
                    ...
                    <div class="form-group">
                        <select name="mission_id" id="mission_id" required>
                            ...
                            @foreach ($missions as $m)
                                <option value="{{ $m->id }}" data-km="{{ $m->trajet->km ?? '' }}"
                                    data-voiture="{{ $m->voiture->id ?? '' }}" data-conso="{{ $m->voiture->conso ?? '' }}">
                                    {{ $m->lieuDepart->nomLieu ?? '' }} - {{ $m->lieuArrive->nomLieu ?? '' }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        <input type="text" id="conso" name="conso" readonly placeholder=" ">
                        <label for="conso">Consommation</label>
                    </div>

                    <div class="form-group">
                        <input type="text" id="km" name="km" readonly placeholder=" ">
                        <label for="km">Kilométrage</label>
                    </div>


                    <div class="form-group">
                        <input type="number" id="prixUnitaire" name="prixUnitaire" step="0.01" required placeholder=" ">
                        <label for="prixUnitaire">Prix unitaire </label>
                    </div>


                    <div class="form-group">
                        <input type="text" id="quantite" name="quantite" readonly placeholder=" ">
                        <label for="quantite">Quantité estimée</label>
                    </div>


                    <div class="form-group">
                        <input type="text" id="cout" name="cout_total" readonly placeholder=" ">
                        <label for="cout">Coût total</label>
                    </div>
                ...
                <h2 class="page-title mt-16">Depense carburant</h2>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function calculer() {
                ...
                if (!isNaN(conso) && !isNaN(km)) {
                    const quantite = (conso / 100) * km;
                    quantiteInput.value = quantite.toFixed(2);

                    if (!isNaN(prix)) {
                        coutInput.value = (quantite * prix).toFixed(2);
                    }
                }
            }
            ...
            missionSelect.addEventListener('change', function() {
                ...
                const conso = selected.getAttribute('data-conso');
                const km = selected.getAttribute('data-km');

                consoInput.value = conso ?? '';
                kmInput.value = km ?? '';
                calculer();
            });
